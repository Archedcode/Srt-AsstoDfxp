...
public class Srt {
	...
	public Srt(ArrayList<String> File,String filepath){
		Srtfile = File;
		Start_End();
		Dialogue();
		new Dfxp(Start,End,Dialogueonly,filepath);
	}
...
//getting start and end stamps
	for(int i =0 ;i<Srtfile.size();i++) 
		if(Srtfile.get(i).matches("[\\d][\\d]:[\\d][\\d]:[\\d][\\d],[\\d][\\d][\\d][\u0020][\u002D][\u002D][\u003E][\u0020][\\d][\\d]:[\\d][\\d]:[\\d][\\d],[\\d][\\d][\\d]"))
	...
	for(int i =0 ;i<Srtfile.size();i++) {
		if(Srtfile.get(i).matches("[\\d][\\d]:[\\d][\\d]:[\\d][\\d],[\\d][\\d][\\d][\u0020][\u002D][\u002D][\u003E][\u0020][\\d][\\d]:[\\d][\\d]:[\\d][\\d],[\\d][\\d][\\d]")) {
			...
			for(int j =i ;j<Srtfile.size();j++) {
				...
				if(Srtfile.get(j).matches("[\\d][\\d]:[\\d][\\d]:[\\d][\\d],[\\d][\\d][\\d][\u0020][\u002D][\u002D][\u003E][\u0020][\\d][\\d]:[\\d][\\d]:[\\d][\\d],[\\d][\\d][\\d]"));
				else
						if(Srtfile.get(j).isEmpty()!=true) {
						lines ++;
						}//if cond
						else
							if(Srtfile.get(j).isEmpty()) {
								if(lines>=2)
									do {
